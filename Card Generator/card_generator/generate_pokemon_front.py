...
def compose_base(stats):
    card_base = 'dark' if stats.trainer in DARK_BASE_ENCOUNTER_TIERS else 'standard'
...
def add_trainer(img, stats):
    ...
    trainer_img = get_img(CARD_ASSETS_DIR / 'trainers' / f'{stats.trainer}.png', xy(9.5, 9.5))
    img.paste(trainer_img, xy(0, 6.75), trainer_img)

    battle_img = get_img(CARD_ASSETS_DIR / 'trainer_icons' / f'{stats.trainer}.png', xy(2, 2))
...
def add_encounter_icon(img, stats):
    ...
    if stats.is_shiny == 1:
        shiny_img = get_img(CARD_ASSETS_DIR / 'encounter_icons' / f'shiny.png', xy(0.75, 0.75))
        img.paste(shiny_img, xy(14.75, 0.5), shiny_img)
...
def add_evolution_icon(img, stats):
    ...
    if not pd.isnull(stats.evolve_into):
        if isinstance(stats.evolve_cost, str) and 'hidden' in stats.evolve_cost.lower():
...
def add_text(img, stats):
    ...
    if not pd.isnull(stats.evolve_cost):
        if isinstance(stats.evolve_cost, str) and 'hidden' in stats.evolve_cost.lower():
            pass
        elif isinstance(stats.evolve_cost, (int, float)):
...
def run(overwrite=False):
    ...
    for i, stats in tqdm(df.iterrows(), total=df.shape[0]):
        output_path = POKEMON_CARD_FRONTS_OUTPUT_DIR / f'{i}_{stats.pokedex_name.lower()}.png'
